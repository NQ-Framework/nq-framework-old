...
describe('JobsService', () => {
    ...
    const mockChanges = [
        { name: 'existing mock job', organizationId: 'test org id', id: 1, active: false, cronInterval: '* * * * *' },
        { name: 'new mock job', organizationId: 'test org id', id: 3, active: true, configuration: { type: 'mock type' }, cronInterval: '* * * * *' }
    ]
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            ...
            providers: [JobsService,
                { provide: SchedulerRegistry, useValue: { deleteCronJob: jest.fn(), getCronJobs: jest.fn().mockImplementation(() => mockExistingJobs), addCronJob: jest.fn() } },
                { provide: HandlerService, useValue: { getHandlerFromConfig: jest.fn() } },
            ]
        }).compile();
    ...
    it('should create jobs from changes snapshot', async () => {
        ...
        expect(registry.addCronJob).toHaveBeenCalledWith('test org id:3', expect.any(Object));
        expect(CronJob).toHaveBeenCalledTimes(1);
        (CronJob as jest.Mock).mock.calls[0][1]();
        expect(executeHandlerMock).toHaveBeenCalledWith({ type: 'mock type' }, expect.objectContaining({ name: 'new mock job' }))
    });
