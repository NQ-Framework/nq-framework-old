...
import { ScheduledJob, StoredProcedureConfiguration } from '@nqframework/models';
...
export class StoredProcedureJobService extends BaseJobService {
    ExecuteJob(config: StoredProcedureConfiguration, job: ScheduledJob): void {
        console.log("this would execute a sp for org " + job.organizationId + " with: ", JSON.stringify(config));
    }
}
