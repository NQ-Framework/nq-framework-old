...
export class OrganizationMiddleware implements NestMiddleware {
    use(req: Request, _: any, next: Function) {
        const subdomain = req.subdomains && req.subdomains.find((sd: string) => sd !== "server");
        if (subdomain) {
            ...
            if (req.query['organizationId']) {
                req.organizationId = getOrganizationIdFromDomain(req.query['organizationId'].toString());
            }
        }
