...
export class WorkflowService {
    ...
    async addActionToWorkflow(actionId: string): Promise<void> {
        const res = await fetch(url + "/workflow/s9JKbcLMio6CVC5K7nHG/actions?organizationId=livona",
            {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
    ...
    async removeActionFromWorkflow(actionName: string): Promise<void> {
        const res = await fetch(url + `/workflow/s9JKbcLMio6CVC5K7nHG/actions/${actionName}?organizationId=livona`,
            {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
