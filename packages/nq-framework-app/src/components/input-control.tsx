...
import { Checkbox, CheckboxProps, Input, InputProps, Select, SelectProps, Textarea, TextareaProps } from "@chakra-ui/react";
...
export const InputControl: React.FC<InputControlProps & (InputProps | SelectProps | TextareaProps | CheckboxProps)> = ({ fieldProperty, ...rest }) => {
    ...
    if (fieldProperty.type === "select-one") {
        if (!fieldProperty.selectOptions || fieldProperty.selectOptions.length === 0) {
            return <div>Invalid field setup, select-one needs to provide selectOptions</div>
        }
        return (<Select {...rest as any}>
            {fieldProperty.selectOptions.map((so: any) => (
                <option key={so.value} value={so.value}>{so.name}</option>
            ))}
        </Select>)
    }
    if (fieldProperty.type === "credentials") {
        if (!organization || !organization.dataCredentials) {
            return <div>Invalid field setup, could not find any credentials for this organization</div>
        }
        return (<Select {...rest as any}>
