...
export const ActionProperties: React.FC<{
  deleteAction: (action: ActionInstance) => Promise<void>;
  deleteLink: (link: ActionLink) => Promise<void>;
  selected: { action: Action; instance: ActionInstance } | null;
  ...
  const [deleteTargetType, setDeleteTargetType] = React.useState<
    (ActionLink & ActionInstance) | null
  >(null);
  ...
  const onDeleteItem = useCallback(() => {
    ...
    if (deleteTargetType?.fromName) {
      deleteLink(deleteTargetType as ActionLink)
        .then(() => {
          setIsDeleteOpen(false);
        })
        .catch(() => {
      ...
      deleteAction(deleteTargetType as ActionInstance)
        .then(() => {
          setIsDeleteOpen(false);
        })
        .catch(() => {
  ...
  useEffect(() => {
    if (workflow && selected) {
      setOutgoingLinks(
        workflow.actionLinks.filter(
          (al) => al.fromName === selected.instance.name,
        ),
      );
      setIncomingLinks(
        workflow.actionLinks.filter(
          (al) => al.toName === selected.instance.name,
        ),
      );
    }
  ...
  const initialValues = convertPropertiesToFormValues(
    selected.instance.configuration.input,
  );

  const incomingLinksComponent =
            ...
            <Form>
              <Stack>
                <FormProperties
                  ...
                  index={null}
                  formikProps={props}
                  propsToRender={selected.action.properties}
                />
                <Button
                  mt={4}
                  colorScheme="teal"
