...
export const NewDataCredentialsPage: React.FC = () => {
  ...
  const [dataCredentialsTypes, setDataCredentialsTypes] = useState<
    DataCredentialsType[]
  >([]);
  ...
  const createDataCredential = useCallback(
    (values: any, cb: Function) => {
      ...
      return service
        .createDataCredentials(dc, organization?.name ?? '')
        .then(() => {
          cb();
          setRedirect(true);
        });
  ...
  const generateFormForType = useCallback(
    (type: string) => {
  ...
  if (redirect) {
    return <Redirect to={{ pathname: '/data-credentials/' }} />;
  }
  return (
    <>
      {!user ? (
                  ...
                  {(props) => (
                    <Form>
                        ...
                        {({ field }: any) => (
                          <FormControl>
                            ...
                            <Select
                              {...field}
                              onChange={(e) => {
                                generateFormForType(e.target.value);
                                props.setFieldValue('type', e.target.value);
                              }}
                              id={'type'}
                              placeholder="Tip kredencijala"
                            >
                      ...
                      {fields &&
                        fields.map((f) => (
                          <Field
                            key={'configuration.' + f.name}
                            name={'configuration.' + f.name}
                          >
                            {({ field }: any) => (
                              <FormControl>
                                <FormLabel htmlFor={'configuration.' + f.name}>
                                  {f.name}
                                </FormLabel>
                                <Input
                                  {...field}
                                  id={'configuration.' + f.name}
                                  placeholder={f.name}
                                />
                              </FormControl>
                            )}
