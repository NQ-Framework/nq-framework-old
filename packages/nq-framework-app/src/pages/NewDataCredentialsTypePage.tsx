...
export const NewDataCredentialsTypePage: React.FC = () => {
  ...
  const createDataCredentialType = useCallback(
  ...
  if (redirect) {
    return <Redirect to={{ pathname: '/data-credentials/types' }} />;
  }
  return (
    <>
      {!user ? (
        ...
        <Layout>
          <GridItem rowSpan={2}>
            <Grid templateColumns="1fr min(120ch, 100%) 1fr" py={12}>
              <GridItem colStart={2}>
                <Formik
                  initialValues={fields.reduce((obj: any, f) => {
                            ...
                            <Box key={f.name}>
                              <FieldArray name={f.name}>
                                {({ push, remove }) => (
                                  <>
                                    {props.values[f.name] &&
                                    props.values[f.name].length > 0 ? (
                                      props.values[f.name].map(
                                        (
                                          arrayFieldInstance: {
                                            name: string;
                                            type: string;
                                          ...
                                          <HStack justify="center">
                                            ...
                                            <Field
                                              key={`${f.name}.${index}.type`}
                                              name={`${f.name}.${index}.type`}
                                            >
                                              {({ field, form }: any) => (
                                                <FormControl
                                                  isInvalid={
                        ...
                        return (
                          <Field key={f.name} name={f.name}>
                            {({ field, form }: any) => (
                              <FormControl
                                isInvalid={
                                  form.errors[f.name] && form.touched[f.name]
                                }
                              >
