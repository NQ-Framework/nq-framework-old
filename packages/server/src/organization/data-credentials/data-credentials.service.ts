...
export class DataCredentialsService {
    ...
    async getCredentialTypes(): Promise<DataCredentialsType[]> {
        ...
        const typesColl = await app.firestore().collection("dataCredentialsTypes").get();
    ...
    async createCredentialType(newType: DataCredentialsType): Promise<void> {
        const app = await getFirebaseApp();
        const existingDoc = await app.firestore().doc(`dataCredentialsTypes/${newType.type}`).get();
        if (existingDoc.exists) {
            throw new Error("credentials type already exists!");
        }
        await app.firestore().collection("dataCredentialsTypes").doc(newType.type).create(newType);
    }
}
