...
describe('executeStack', async () => {
  ...
  it('should execute an action from the top of the stack', async () => {
    mockExecuteAction.mockImplementation((action: ActionInstance) => ({
      propertyValues: [],
      outputValues: [
        { name: 'mock output', value: 'mock output value' + action.name },
      ],
    }));
    ...
    expect(mockExecuteAction).toHaveBeenCalledWith(
      context.workflow.actionInstances[0],
      context,
    );
    expect(
      context.actions[context.workflow.actionInstances[0].name],
    ).toBeDefined();
    expect(
      context.actions[context.workflow.actionInstances[1].name],
    ).toBeDefined();
    expect(context.input['mock output']).toEqual(
      'mock output value' + context.workflow.actionInstances[1].name,
    );
    ...
    expect(mockExecuteAction).toHaveBeenCalledWith(
      context.workflow.actionInstances[1],
      context,
    );
    expect(context.stack).toEqual([]);
  });
  ...
  it('should execute only one action if link to other is invalid', async () => {
    ...
    context.workflow.actionLinks = [
      { ...context.workflow.actionLinks[0], toName: 'invalid name' },
    ];
    context.stack.push(context.workflow.actionInstances[0]);
    await executeStack(context, mockActionService);
