...
export const handler: ActionHandler = {
  handle: async (
    ...
    const credentialsName = propertyValues.find((i) => i.name === 'credentials')?.value;
    if (!credentialsName) {
      throw new Error('Missing required parameter message');
    }
    const credentials = workflowExecution.organization.dataCredentials.find(dc => dc.name === credentialsName);
    if (!credentials) {
      throw new Error(`Could not find credentials: ${credentialsName} in organization: ${workflowExecution.organization.name}`);
    }

    const credentialConfig = reducePropertyValuesToObject(credentials.configuration);
    const apiCallConfig = reducePropertyValuesToObject(propertyValues);

    const token = await evaluateExpression(credentialConfig.getTokenExpression, {
      actionInstance,
      propertyValues,
      workflowExecution,
      fetch,
