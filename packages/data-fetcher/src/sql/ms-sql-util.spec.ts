...
describe("MsSqlUtils", () => {
    ...
    expect(mockConnection).toHaveBeenCalledWith({
      ...
      options: {
        trustServerCertificate:
          mockConnectionConfiguration.trustServerCertificate,
  ...
  it("should throw error received from db", async () => {
    ...
    await expect(
      connect(mockConnectionConfiguration)
  ...
  it("should execute sql based on props", async () => {
    ...
    expect(response).toEqual({
      rowCount: 1,
      rows: [{ "test column name": "test value" }],
      outParams: {},
    });
    expect(addParamMock).toHaveBeenCalledWith(
      mockParam.name,
      mockParam.type,
      mockParam.value
    );
  ...
  it("should execute procedure based on props", async () => {
    ...
    expect(response).toEqual({
      rowCount: 1,
      rows: [{ "test column name": "test value" }],
      outParams: { "test output param name": 1 },
    });
    expect(addParamMock).toHaveBeenCalledWith(
      mockParam.name,
      mockParam.type,
    ...
    expect(addOutputParamMock).toHaveBeenCalledWith(
      mockOutputParam.name,
      mockOutputParam.type
    );
  });
  it("should reject if sql returns error", async () => {
    ...
    await expect(
      execute(mockProps, conInstance as any)
    ).rejects.toThrowErrorMatchingSnapshot();
    expect(conInstance.callProcedure).toHaveBeenCalledTimes(1);
    expect(conInstance.close).toHaveBeenCalledTimes(1);
  });
